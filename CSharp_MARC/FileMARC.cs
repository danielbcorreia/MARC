...
{
    /// <summary>
    /// The main FileMARC class enables you to return Record objects from a stream or string.
    /// </summary>
    public class FileMARC : IEnumerator, IEnumerable
    {
        ...
        // HEX value for End of File
        private const string END_OF_FILE = "\x1A";
        ...
        private readonly List<string> _rawSource;
        ...
        public void Add(string source)
        {
            ...
            foreach (string record in source.Split(END_OF_RECORD))
            {
				...
				if (record != string.Empty && record != END_OF_FILE)
				{
					// split removes the END_OF_RECORD. Put it back in
					_rawSource.Add(record + END_OF_RECORD);
				}
            }
        }
